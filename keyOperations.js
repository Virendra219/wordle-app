...
for (let i = 0; i<keyboardButtons.length; i++) {
    keyboardButtons[i].addEventListener("click", () => {clickHandler(keyboardButtons[i].innerHTML)});
}

document.getElementById("w1-l1").focus();

function clickHandler(value) {
    const curID = "w" + w + "-l" + l;
    const current = document.getElementById(curID);
    current.innerHTML = value;
    l = (l==5 ? 5 : l+1)
    const newID = "w" + w + "-l" + l;
}

document.getElementById("Back").addEventListener("click", () => {DeleteHandler()});

function DeleteHandler() {
    const curID = "w" + w + "-l" + l;
    const curCell = document.getElementById(curID);
    if (curCell.innerHTML === "") {
        l = (l==1 ? l : l-1);
        const prevCellId = "w" + w + "-l" + l;
...
function SubmitHandler() {
    if (l!=5 || document.getElementById("w" + w + "-l5").innerHTML === "") {
        alert("Enter a five letter word");
        document.getElementById("w" + w + "-l" + l).focus();
        return;
    }
    let inputWord = "";
    for (let i=1; i<=5; i++) {
        const cell = document.getElementById("w" + w + "-l" + i);
        if (cell.innerHTML === correctWord[i-1]) {
            cell.style.backgroundColor = "rgb(4, 180, 4)";
            const key = document.getElementById(cell.innerHTML);
            key.style.backgroundColor = "rgb(4, 180, 4)";
        }
        else {
            let flag = false;
            for (let j=0; j<5; j++) {
                if (cell.innerHTML === correctWord[j]) {
                    flag = true;
                    break;
                }
            }
            if (flag) {
                cell.style.backgroundColor = "rgb(248, 212, 6)";
                const key = document.getElementById(cell.innerHTML);
                key.style.backgroundColor = "rgb(248, 212, 6)";
            }
            else {
        ...
        inputWord += cell.innerHTML;
    }
    if (inputWord === correctWord)
    {
        for (let i=1; i<=5; i++) {
            document.getElementById("w"+w+"-l"+i).style.backgroundColor = "rgb(4, 180, 4)";
        }
        alert("You win!");
        PlayAgain();
        return;
...
function PlayAgain() {
    for (w; w>=1; w--) {
        for (l; l>=1; l--) {
            const cellId = "w" + w + "-l" + l;
            const cell = document.getElementById(cellId);
            cell.style.backgroundColor = "rgb(210, 221, 224)";
            const key = document.getElementById(cell.innerHTML);
            key.style.backgroundColor = "rgb(210, 221, 224)";
            cell.innerHTML = "";
        }
        l=5;
    }
